...
const handleDelete = (e) => {
    async function deleteData() {
        const res = await axios.delete("http://localhost:3030/student/" + e.target.id);
        return res.status; // (Or whatever)
    }
    console.log("Detele status: " + deleteData());
    // axios.post("http://localhost:3030/student", { id: 3, name: "Tuan" });
    // axios.put("http://localhost:3030/student/" + e.target.id, { name: "Tuan2" });
...
export default function StudentHome({ studentList }) {
    ...
    return (
        <div className="container pt-5">
            <h2>Students list</h2>
            <button onClick={(e) => router.push('/student-manager/create')} className="mx-4 btn btn-success">
                Create new student
            </button>
            <ul>
